    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.projects.update", [$project->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('location') ? 'is-invalid' : '' }}" name="location_id" id="location_id" required>
                    @foreach($locations as $id => $location)
                        <option value="{{ $id }}" {{ (old('location_id') ? old('location_id') : $project->location->id ?? '') == $id ? 'selected' : '' }}>{{ $location }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('project_types') ? 'is-invalid' : '' }}" name="project_types[]" id="project_types" multiple>
                    @foreach($project_types as $id => $project_types)
                        <option value="{{ $id }}" {{ (in_array($id, old('project_types', [])) || $project->project_types->contains($id)) ? 'selected' : '' }}>{{ $project_types }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('features') ? 'is-invalid' : '' }}" name="features" id="features">{{ old('features', $project->features) }}</textarea>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_featured') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="is_featured" id="is_featured" value="1" {{ $project->is_featured || old('is_featured', 0) === 1 ? 'checked' : '' }}>
